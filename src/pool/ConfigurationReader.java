...
/**
 * 这个类是为了读取properties配置文件而设置的
 * 整个项目启动的时候读取一次就够了
 * 将文件中的数据临时存储在一个Map集合中----缓存
 * */
public class ConfigurationReader {
    ...
    static {
        try {
            ...
            Enumeration en = pro.propertyNames();//相当于map集合中的keySet()方法
            //en的使用方式类似Iterator  ResultSet
            while(en.hasMoreElements()){
                String key = (String)en.nextElement();
                String value = pro.getProperty(key);//相当于map集合的get方法
                map.put(key, value);
            }
        } catch (IOException e) {
            e.printStackTrace();
